...
int main(int argc, char** argv){
    ...
    while(ros::ok()){
        ...
        if (newFlexxPointCloudLast){
            ...
            pcl::removeNaNFromPointCloud(*FlexxPointCloudLast,*FlexxPointCloudLast,indices_src);
            std::cout <<"After removed NAN from pointcloud: "<<FlexxPointCloudLast->size()<< std::endl;
            //截取船模部分有效点云，使用条件滤波
            ...
            //if(abs(data[10]-data[20]))
            float zmin = data[10]-0.1;
	        float zmax = data[10]+0.1;
            ...
            pcl::toROSMsg(*cloud_src_voxelgrid,FlexxPointCloudAfterfilted);
            ...
            pubFlexxPointCloudAfterMatched.publish(FlexxPointCloudAfterMatched);
